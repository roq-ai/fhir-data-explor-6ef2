...
import { getObservations, deleteObservationById } from 'apiSdk/observations';
import { ObservationInterface } from 'interfaces/observation';

type ColumnType = ColumnDef<ObservationInterface, unknown>;
...
export function ObservationListPage(props: ObservationListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getObservations({
        ...
        order: params.order,
        searchTermKeys: ['type.contains', 'result.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<ObservationInterface>>(
    () => `/observations?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: ObservationInterface) => {
    if (hasAccess('observation', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/observations/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/observations/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('observation', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/observations/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('observation', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="observation" />
        </Flex>

        {hasAccess('observation', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/observations/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
