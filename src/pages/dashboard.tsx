...
import { getObservations } from 'apiSdk/observations';
import { ObservationInterface } from 'interfaces/observation';
import { getMedications } from 'apiSdk/medications';
import { MedicationInterface } from 'interfaces/medication';
import { getEncounters } from 'apiSdk/encounters';
import { EncounterInterface } from 'interfaces/encounter';

import { MedicationListPage as ShowCaseList } from 'pages/medications';

function DashboardPage() {
  const { session } = useSession();

  const observationStat = useSWR<PaginatedInterface<ObservationInterface>>(
    () => `/observations`,
    () => getObservations({ limit: 1, offset: 0 }),
  );

  const medicationStat = useSWR<PaginatedInterface<MedicationInterface>>(
    () => `/medications`,
    () => getMedications({ limit: 1, offset: 0 }),
  );

  const encounterStat = useSWR<PaginatedInterface<EncounterInterface>>(
    () => `/encounters`,
    () => getEncounters({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Observations',
      result: observationStat?.data?.totalCount,
      isLoading: observationStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Medications',
      result: medicationStat?.data?.totalCount,
      isLoading: medicationStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Encounters',
      result: encounterStat?.data?.totalCount,
      isLoading: encounterStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`medications`} passHref legacyBehavior>
                  <Button
