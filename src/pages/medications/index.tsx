...
import { getMedications, deleteMedicationById } from 'apiSdk/medications';
import { MedicationInterface } from 'interfaces/medication';

type ColumnType = ColumnDef<MedicationInterface, unknown>;
...
export function MedicationListPage(props: MedicationListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getMedications({
        ...
        order: params.order,
        searchTermKeys: ['name.contains', 'dosage.contains', 'instructions.contains', 'dispensing_details.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<MedicationInterface>>(
    () => `/medications?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: MedicationInterface) => {
    if (hasAccess('medication', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/medications/view/${row.id}`);
    }
  };

  const columns: ColumnType[] = [
    { id: 'name', header: 'Name', accessorKey: 'name' },
    { id: 'dosage', header: 'Dosage', accessorKey: 'dosage' },
    { id: 'instructions', header: 'Instructions', accessorKey: 'instructions' },
    { id: 'dispensing_details', header: 'Dispensing Details', accessorKey: 'dispensing_details' },
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/medications/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('medication', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/medications/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="medication" />
        </Flex>

        {hasAccess('medication', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/medications/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
