...
import { getEncounters, deleteEncounterById } from 'apiSdk/encounters';
import { EncounterInterface } from 'interfaces/encounter';

type ColumnType = ColumnDef<EncounterInterface, unknown>;

interface EncounterListPageProps {
...
export function EncounterListPage(props: EncounterListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<EncounterInterface>>(
    () => `/encounters?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: EncounterInterface) => {
    if (hasAccess('encounter', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/encounters/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/encounters/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('encounter', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/encounters/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('encounter', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="encounter" />
        </Flex>

        {hasAccess('encounter', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/encounters/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
