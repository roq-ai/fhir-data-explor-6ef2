...
import { getEncounterById } from 'apiSdk/encounters';
import { EncounterInterface } from 'interfaces/encounter';

function EncounterViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<EncounterInterface>(
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('encounter', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/encounters/edit/${id}`} passHref legacyBehavior>
                    <Button
