...
export function PatientListPage(props: PatientListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getPatients({
        relations: ['organization', 'encounter.count', 'medication.count', 'observation.count'],
  ...
  const columns: ColumnType[] = [
    ...
    { id: 'last_name', header: 'Last Name', accessorKey: 'last_name' },
    {
      ...
      cell: ({ row: { original: record } }: any) =>
        record?.dob ? format(parseISO(record?.dob as unknown as string), 'dd-MM-yyyy') : '',
    },
    ...
    hasAccess('organization', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ...
      : null,
    hasAccess('encounter', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'encounter',
          header: 'Encounter',
          accessorKey: 'encounter',
          cell: ({ row: { original: record } }: any) => record?._count?.encounter,
        }
      : null,
    hasAccess('medication', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          cell: ({ row: { original: record } }: any) => record?._count?.medication,
        }
      : null,
    hasAccess('observation', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'observation',
          header: 'Observation',
          accessorKey: 'observation',
          cell: ({ row: { original: record } }: any) => record?._count?.observation,
        }
      : null,
